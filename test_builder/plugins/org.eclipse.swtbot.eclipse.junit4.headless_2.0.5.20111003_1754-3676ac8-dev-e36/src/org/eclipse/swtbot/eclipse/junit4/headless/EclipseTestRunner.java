...
public class EclipseTestRunner implements TestListener {
	...
	public static int run(String[] args) throws IOException {
		...
		// Added option to modify test class. This <preClassName> class could modify the test class using byte manipulation technology.
		...
		for (int i = startArgs; i < args.length; i++)
			if (args[i].toLowerCase().equals("-classname")) { //$NON-NLS-1$
				if (i < args.length - 1)
					className = args[i + 1];
				i++;
			} else if (args[i].toLowerCase().equals("-preclassname")) { //$NON-NLS-1$
                if (i < args.length - 1)
                    preClassName = args[i + 1];
                i++;
		...
		try {
		    // It must be loaded prior to <className> class. If not, the JVM would load the original class.
		    Class<?> cls = Platform.getBundle(testPluginName).loadClass(preClassName);
		    cls.newInstance(); // to call static initializer
        } catch (Exception e) {
            System.err.println("Failed to instantiate <preClassName> class:" + e.toString());
        }
